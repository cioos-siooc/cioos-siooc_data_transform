...
class TestBIOODFConversion(unittest.TestCase):
    def test_bio_odf_sample_data_conversion(self):
        ...
        default_odf_config = odf_transform.process.read_config("./projects/odf_transform/config.json")
        sample_files = glob("./projects/odf_transform/sample_data/bio_samples/**/*.ODF",recursive=True)
    ...
    def test_mli_odf_sample_data_conversion(self):
        ...
        default_odf_config = odf_transform.process.read_config("./projects/odf_transform/config.json")
        sample_files = glob("./projects/odf_transform/sample_data/mli_samples/**/*.ODF",recursive=True)
    ...
    def test_bio_odf_converted_netcdf_vs_references(self):
        nc_files = glob("./projects/odf_transform/sample_data/bio_samples/**/*.ODF_reference.nc",recursive=True)
        for nc_file in nc_files:
            ...
            ref.attrs['history'] = re.sub(timestamp_format,'TIMESTAMP',ref.attrs['history'])
            test.attrs['history'] = re.sub(timestamp_format,'TIMESTAMP',test.attrs['history'])

            if not ref.identical(test):
                raise RuntimeError(f"Converted file {nc_file_test} is different than the reference: {nc_file}")
