...
def write_bcsop_ncfile(filename, profile_id, sopdf, status):
    ...
    out.institution = (
        "Institute of Ocean Sciences, 9860 West Saanich Road, Sidney, B.C., Canada"
    )
    out.infoUrl = (
    ...
    ncfile_var_list.append(
        OceanNcVar(
            ...
            None,
            None,
            "British Columbia Shore station Observation Program (BCSOP)",
        )
    )
    ncfile_var_list.append(
        OceanNcVar("str_id", "contact_name", None, None, None, "Peter Chandler")
    )
    ncfile_var_list.append(
        OceanNcVar(
            "str_id", "contact_email", None, None, None, "peter.chandler@dfo-mpo.gc.ca"
        )
    )
    ncfile_var_list.append(
        OceanNcVar("str_id", "agency", None, None, None, "Fisheries and Oceans Canada")
    )
    ncfile_var_list.append(
        OceanNcVar(
            "str_id",
            ...
            None,
            None,
            "Given this is a multi-decade time series the sampling instruments have changed over time. At present measurements are made with a YSI Pro30 multimeter.",
        )
    )
    ncfile_var_list.append(
        OceanNcVar(
            "lat", "latitude", "degrees_north", None, None, sopdf["latitude"].values[0]
        )
    )
    ncfile_var_list.append(
        OceanNcVar(
            "lon", "longitude", "degrees_east", None, None, sopdf["longitude"].values[0]
        )
    )
    ncfile_var_list.append(
    ...
    ncfile_var_list.append(
        OceanNcVar("time", "time", None, None, None, obs_time_utc, vardim=("time"))
    )
    ...
    ncfile_var_list.append(
        OceanNcVar(
            ...
            "TEMPTC01",
            "deg C",
            sopdf["temperature"].min,
            sopdf["temperature"].max,
            sopdf["temperature"].values,
            ncfile_var_list,
            ("time"),
            null_value,
            conv_to_BODC=False,
