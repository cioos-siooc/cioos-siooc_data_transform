...
def readLocDict(fname):
    data = np.genfromtxt(
        fname, skip_header=2, usecols=(0, 3, 4), dtype=str, delimiter=","
    )
    locDict = {}
    for i, loc in enumerate(data[:, 0]):
        locDict[loc.replace(" ", "_").lower()] = {
            "lat": float(data[i, 1]),
            "lon": float(data[i, 2]),
        }
    return locDict
...
for fname in [
    "DATA_-_Active_Sites.zip",
    "DATA_-_Archived_Sites.zip",
    "DATA_-_In-active_Sites.zip",
]:
    with ZipFile(datapath + fname) as myzipfile:
        for f in myzipfile.namelist():
            if (
                "Daily_Sea_Surface_Temperature_and_Salinity" in f
                and ".csv" in f
                and "french" not in f
            ):
...
for fname in [
    ...
    with ZipFile(datapath + fname) as myzipfile:
        for f in myzipfile.namelist():
            if (
                "Average_Monthly_Sea_Surface_Temp" in f
                and ".csv" in f
                and "french" not in f
            ):
                try:
                    loc, dfTemp = read_monthly_avg(
                        myzipfile.open(f), varname="temperature"
                    )
                    loc, dfSalt = read_monthly_avg(
                        myzipfile.open(f.replace("Temperatures", "Salinities")),
                        varname="salinity",
                    )
