...
def _create_new_empty_variable(
    dset, new_variable_name, similar_variables, long_name, standard_name, units
):
    ...
    if multiple_variables:
        ...
        if new_variable_name not in dset.variables.keys():
            dset.createVariable(
                new_variable_name, variable_type, variable_dimensions, fill_value=np.nan
            )
...
def add_standard_variables(filename):
    ...
    history_attribute = {
        _get_time_stamp(): "Standardize ocean variables with add_standard_name tool"
    }
    ...
    if _has_variable(combine_var_names, variable_list):
        ...
        # Define History
        add_history_line = (
            "Create " + new_variable + " variable and apply the following parameters: "
        )

        if (
            variable_name_convention == "BODC"
        ):  # If use BODC convention for variable names
            ...
            if "TEMPS601" in variable_list:
                ...
                var, is_updated = _fill_nan(
                    var, gsw.t90_from_t68(dset.variables["TEMPS601"][:])
                )
                if is_updated:
                    add_history_line = (
                        add_history_line + "TEOS-10 gsw.t90_from_t68('TEMPS601'), "
                    )
            if "TEMPS602" in variable_list:
                # Convert Secondary Temperature Sensor Data from IPTS-68 to ITS-90
                var, is_updated = _fill_nan(
                    var, gsw.t90_from_t68(dset.variables["TEMPS602"][:])
                )
                if is_updated:
                    add_history_line = (
                        add_history_line + "TEOS-10 gsw.t90_from_t68('TEMPS602'), "
                    )
        ...
        history_attribute[_get_time_stamp()] = add_history_line[
            :-2
        ]  # ignore the last ','
    ...
    if _has_variable(combine_var_names, variable_list):
        ...
        if (
            ...
            if (
                "SSALST01" in variable_list
            ):  # Convert Primary Salinity Data from IPTS-68 to ITS-90
                var, is_updated = _fill_nan(
                    var, gsw.SP_from_SK(dset.variables["SSALST01"][:])
                )
                if is_updated:
                    add_history_line = (
                        add_history_line + "TEOS-10 gsw.SP_from_SK('PSALBST2'), "
                    )
            if (
                "SSALST02" in variable_list
            ):  # Convert Seconday Salinity Data from IPTS-68 to ITS-90
                var, is_updated = _fill_nan(
                    var, gsw.SP_from_SK(dset.variables["SSALST02"][:])
                )
                if is_updated:
                    add_history_line = (
                        add_history_line + "TEOS-10 gsw.SP_from_SK('SSALST02'), "
                    )
            if (
                "ODSDM021" in variable_list
            ):  # Convert Secondary Salinity Data from IPTS-68 to ITS-90
                var, is_updated = _fill_nan(
                    var, gsw.SP_from_SK(dset.variables["ODSDM021"][:])
                )
                if is_updated:
                    add_history_line = (
                        add_history_line + "TEOS-10 gsw.SP_from_SK('ODSDM021'), "
                    )
    ...
    if _has_variable(combine_var_names, variable_list):
        ...
        _create_new_empty_variable(
            ...
            combine_var_names,
            "Depth in meters",
            "depth_below_sea_level_in_meters",
            "m",
        )
        var = dset[new_variable]
        ...
        if variable_name_convention == "BODC":
            ...
            if "PRESPR01" in variable_list and "latitude" in variable_list:
                var, is_updated = _fill_nan(
                    var,
                    -gsw.z_from_p(
                        dset.variables["PRESPR01"][:], dset.variables["latitude"][:]
                    ),
                )
                if is_updated:
                    add_history_line = (
                        add_history_line
                        + "TEOS-10 gsw.z_from_p('PRESPR01','latitude'), "
                    )
            ...
            if "PRESPR02" in variable_list and "latitude" in variable_list:
                var, is_updated = _fill_nan(
                    var,
                    -gsw.z_from_p(
                        dset.variables["PRESPR02"][:], dset.variables["latitude"][:]
                    ),
                )
                if is_updated:
                    add_history_line = (
                        add_history_line
                        + "TEOS-10 -gsw.z_from_p('PRESPR02','latitude'), "
                    )
            ...
            if len(add_history_line) > 1:
                add_history_line = (
                    "Create "
                    + new_variable
                    + " variable and apply the following parameters: "
                    + add_history_line
                )
                # Append list of variables added to history_attribute
                history_attribute[_get_time_stamp()] = add_history_line[
                ...
                dset[new_variable].comment = str(
                    {_get_time_stamp(): add_history_line[:-2]}
                )
        ...
        if (
            ...
            if "depth" in variable_list and "latitude" in variable_list:
                var, is_updated = _fill_nan(
                    var,
                    gsw.p_from_z(
                        -dset.variables["depth"][:], dset.variables["latitude"][:]
                    ),
                )
                if is_updated:
                    add_history_line = (
                        add_history_line + "TEOS-10 gsw.p_from_z(-'depth','latitude'), "
                    )
