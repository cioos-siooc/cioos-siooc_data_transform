...
def add_variable_attributes(ds, review_attributes=None, overwrite=True):
    """
        ...
        for scale_name in scales:
            for att in review_attributes:
                if att in ds[var].attrs and ds[var].attrs[att]:
                    matched_scale = re.search(
                        scales[scale_name], ds[var].attrs[att], re.IGNORECASE
                    )
...
def generate_bodc_variables(ds):
    ...
    def _read_bodc(urn):
        ...
        if bodc["code_full"] and re.match(r"[^1-9]", bodc["code_full"].strip()[-1]):
            bodc["code_no_trailing"] = "{0}{1}".format(
                bodc["code_no_trailing"], bodc["code_full"][-1]
            )
        return bodc

    # Get Reference Document
    bodc_var = pd.read_csv(
        os.path.join(os.path.split(__loader__.path)[0], "bodc_generator.csv")
    )
    ...
    for var in ds:
        ...
        if sdn_parameter_urn_dict["code_no_trailing"]:
            matched_sdn_p01 = bodc_var["SDN:P01::urn"].str.contains(
                sdn_parameter_urn_dict["code_no_trailing"]
            )
        else:
        ...
        if (
            standard_name == ""
            and sdn_parameter_urn != ""
            and sdn_parameter_urn_dict["code_full"] in bodc_var["SDN:P01::urn"].tolist()
        ):
        ...
        for index, row in matched_bodc.iterrows():
            ...
            match_instrument = _is_matching(
                row["instrument"], ds[var].attrs.get("instrument")
            ) or _is_matching(row["instrument"], ds.attrs.get("instrument"))
            ...
            if match_units and match_scale and match_instrument:
                ...
                ds[var].attrs["matching_sdn_parameter_urn"] = _join_attributes(
                    ds[var].attrs.get("matching_sdn_parameter_urn"),
                ...
                if row["Generate Variable"]:
                    ...
                    if sdn_parameter_urn_dict["trailing_number"]:
                        new_var = "{0}{1}".format(
                            new_var, sdn_parameter_urn_dict["trailing_number"]
                        )
                    ds[new_var] = ds[var]
                    ds[new_var].attrs["original_variable"] = _join_attributes(
                        ds[var].attrs.get("original_variable"), var
                    )
